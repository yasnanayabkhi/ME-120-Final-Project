...
def check_versions(str_filename, ffdoc_filename):
    print("-- Skipping strict CGenFF version parsing (robust mode enabled)")
    print("-- Using CHARMM36-JUL2022 force field assumptions")

    strver = "4.6"
    ffver = "4.6"

    print("-- Assumed CGenFF version (stream file):", strver)
    print("-- Assumed CGenFF version (force field):", ffver)

    print("\nNOTE: Version check bypassed to ensure compatibility with modern CHARMM36 files.\n")
...
nx_ver = version.parse(nx.__version__)

if nx_ver < version.parse("2.0"):
    print("Your NetworkX version is: ", nx.__version__)
    print("This script requires NetworkX 2.x")
    print("Incompatible version.")
    exit()

elif nx_ver > version.parse("2.4"):
    print("Warning: This script was tested up to NetworkX 2.3–2.4 range.")
    print("You are using:", nx.__version__)
    print("Proceeding anyway, but behavior may vary.")
